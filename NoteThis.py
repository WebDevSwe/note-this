...
TOKENS_CONFIG_PATH = Path(__file__).parent / "settings" / "tokens.json"
TOOLTIPS_CONFIG_PATH = Path(__file__).parent / "settings" / "tooltips.json"
ABOUT_MARKDOWN_PATH = Path(__file__).parent / "settings" / "about_notethis.md"
FILE_PREFIX = "note_A"
FILE_SUFFIX = ".md"
...
def load_token_config() -> dict:
    ...
    try:
        token_config = json.loads(TOKENS_CONFIG_PATH.read_text(encoding="utf-8"))
...
def load_tooltips_config() -> dict:
    global tooltips_config
    if tooltips_config is not None:
        return tooltips_config

    try:
        tooltips_config = json.loads(TOOLTIPS_CONFIG_PATH.read_text(encoding="utf-8"))
    except (FileNotFoundError, json.JSONDecodeError):
        tooltips_config = {"enabled": False, "buttons": {}}
    return tooltips_config


def tooltip_text(key: str, fallback: str) -> str:
    config = load_tooltips_config()
...
def toggle_theme(window: tk.Tk, button: tk.Button) -> None:
    global current_theme_name

    current_theme_name = "dark" if current_theme_name == "light" else "light"
    button.config(text="☀" if current_theme_name == "dark" else "🌙")
    apply_theme(window)
    set_status(f"Tema: {'Mörkt' if current_theme_name == 'dark' else 'Ljust'}")
...
def extract_note_id(file_path: Path | None) -> str:
    ...
    for token_group in tokens_config.values():
        ...
        for token_name, token_spec in token_group.items():
            ...
            if source in {"date", "time", "datetime"}:
                ...
                values[token_name] = extract_note_id(file_path)
...
def apply_tokens(text: str, file_path: Path | None, created_at: datetime | None, updated_at: datetime) -> str:
    token_values = build_token_values(file_path, created_at, updated_at)
...
def cycle_ui_scale() -> None:
    ...
    ui_scale_index = (ui_scale_index + 1) % len(UI_SCALE_FACTORS)
...
def save_note(show_empty_warning: bool = True, autosave: bool = False) -> bool:
    ...
    if current_note_file is None:
        current_note_file = next_note_file()

    resolved_text = apply_tokens(
        text=text,
    ...
    write_note_file(current_note_file, resolved_text, create_backup=autosave)
...
def note_list_label(file_path: Path, max_chars: int = 60) -> str:
    ...
    return f"{file_path.name} - {preview}"
...
def open_notes_dialog(window: tk.Tk) -> None:
    ...
    def refresh_list() -> None:
        ...
        for file_path in note_files:
            listbox.insert(tk.END, note_list_label(file_path))
...
def main() -> None:
    ...
    save_as_button = tk.Button(controls, text="Spara som..", command=save_note_as_copy, width=12)
    save_as_button.pack(side="left", padx=(8, 0))
    attach_tooltip(save_as_button, "main.save_as", "Temporär tooltip: Spara kopia som ny anteckningsfil.")
    ...
    zoom_button = tk.Button(right_actions, text="+", command=cycle_ui_scale, width=3)
    zoom_button.pack(side="right", padx=(0, 6))
    attach_tooltip(zoom_button, "main.zoom", "Temporär tooltip: Växla zoom 100%, 150% och 200%.")

    theme_button = tk.Button(right_actions, text="🌙", command=lambda: toggle_theme(window, theme_button), width=3)
    theme_button.pack(side="right", padx=(0, 6))
    attach_tooltip(theme_button, "main.theme", "Växla mellan ljust och mörkt tema.")

    about_button = tk.Button(right_actions, text="?", command=lambda: open_about_dialog(window), width=3)
    about_button.pack(side="right", padx=(0, 6))
    attach_tooltip(about_button, "main.about", "Visa information om NoteThis.")

    timestamp_button = tk.Button(right_actions, text="🕒", command=insert_timestamp, width=3)
    timestamp_button.pack(side="right")
    attach_tooltip(timestamp_button, "main.timestamp", "Temporär tooltip: Infoga aktuell tid vid markören.")
